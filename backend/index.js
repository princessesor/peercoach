...
mongoose
  ...
  .then(() => {
    ...
    const users = {};
    io.on("connection", (socket) => {  //for listening to events
      ...
      socket.on("sendMessage", async (data) => {
        ...
        try {
          ...
          const receiverSocket = Object.keys(users).find(
            (key) => users[key].toString() === data.receiverId.toString()
          );          
...
app.post("/updateProfile", checkAuthenticated, async (req, res) => {
  ...
  try {
    ...
    user.skillsDesired = skillsDesired
      .split(",")
      .map((skill) => skill.trim()); // Split and trim skills
...
app.get("/matchesPage", checkAuthenticated, async (req, res) => {
  try {
    ...
    const matches = allUsers.filter((user) => {
      const hasMatchingSkill = user.skillsHave.some((skill) =>
